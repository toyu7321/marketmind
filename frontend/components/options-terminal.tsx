...
export function OptionsTerminal() {
  ...
  const expirations = [...new Set(data.chain.map((item: Data) => item.expiration))] as string[];
  const rows = data.chain.filter((item: Data) => (type === 'ALL' || item.type === type) && (expiry === 'ALL' || item.expiration === expiry));
  ...
  return <><div className="page-heading"><div><span className="eyebrow">DERIVATIVES / RISK-DEFINED ANALYSIS</span><h1>Options Terminal</h1><p>Chain data, Greeks when supplied, and deterministic payoff summaries.</p></div><div className="heading-actions"><label className="symbol-input"><Search/><input value={symbol} maxLength={8} onChange={event => setSymbol(event.target.value.toUpperCase())}/></label></div></div>
    <div className="metric-strip"><div><span>UNDERLYING</span><b>{money(data.spot)}</b></div><div><span>IV SUMMARY</span><b>{display(data.iv_rank, '%')}</b></div><div><span>PUT / CALL</span><b>{display(data.put_call_ratio)}</b></div><div><span>DATA</span><Badge tone={data.mode === 'LIVE' ? 'good' : 'warn'}>{data.mode}</Badge></div></div>
    ...
    <div className="options-grid"><Panel title={data.symbol + ' Option Chain'} eyebrow={rows.length + ' CONTRACTS · ' + data.mode + ' SOURCE'} className="table-panel"><div className="table-scroll"><table><thead><tr>{['Expiry', 'Strike', 'Type', 'Bid', 'Ask', 'Last', 'Volume', 'Open Int.', 'IV', 'Delta', 'Gamma', 'Theta', 'Vega'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((item: Data, index: number) => <tr key={(item.contract || item.expiration) + '-' + item.strike + '-' + item.type + '-' + index}><td>{item.expiration}</td><td><b>{item.strike}</b></td><td><Badge tone={item.type === 'CALL' ? 'good' : 'bad'}>{item.type}</Badge></td><td>{display(item.bid)}</td><td>{display(item.ask)}</td><td>{display(item.last)}</td><td>{display(item.volume)}</td><td>{display(item.open_interest)}</td><td>{display(item.iv, '%')}</td><td>{display(item.delta)}</td><td>{display(item.gamma)}</td><td className="negative">{display(item.theta)}</td><td>{display(item.vega)}</td></tr>)}</tbody></table></div></Panel>
