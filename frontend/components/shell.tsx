...
import {clearClientDataCache, prefetchApiData, useApiData} from '@/lib/data';
import {markNavigationRenderComplete, markNavigationStart} from '@/lib/performance';
...
export function Shell({children}: {children: React.ReactNode}) {
  ...
  useEffect(() => { markNavigationRenderComplete(path); }, [path]);
  useEffect(() => {
  ...
  const navigate = (target: string) => { markNavigationStart(target); router.push(target); setPalette(false); };
  const prefetchForRoute = (target: string) => {
    const dataPath = target === '/markets' ? '/dashboard' : target === '/options' ? null : target === '/' ? '/dashboard' : target;
    router.prefetch(target);
    if (dataPath) void prefetchApiData(dataPath);
  };
  ...
  return <div className={appClass}><aside className={mobile ? 'open' : ''}>
    ...
    <nav>{visibleNav.map(([href, label, Icon]) => <Link key={href} href={href} className={path === href || (href !== '/' && path.startsWith(href)) ? 'active' : ''} onMouseEnter={() => prefetchForRoute(href)} onFocus={() => prefetchForRoute(href)} onClick={() => { markNavigationStart(href); setMobile(false); }}><Icon/><span>{label}</span><ChevronRight className="chev"/></Link>)}</nav>
    ...
    <header className="topbar"><button className="menu" onClick={() => setMobile(true)}><Menu/></button><div className="statuses"><span title={health?.last_successful_request||'No successful market request yet'}><i className={health?.market_data_status === 'Healthy' ? '' : health?.market_data_status === 'Demo fallback' ? 'warn' : 'off'}/> MARKET DATA · {health?.market_provider === 'Alpaca' ? `ALPACA · ${(health.market_feed||'').toUpperCase()} · ${health.market_data_status||'CONFIGURED'}` : health?.market_data_status === 'Demo fallback' ? 'DEMO FALLBACK' : 'CHECKING'}</span><span><i className={health?.authentication === 'configured' ? '' : 'warn'}/> {health?.authentication === 'configured' ? 'AUTH SECURED' : 'AUTH CHECKING'}</span><span><i className="warn"/> AI RULES</span><span><i/> BROKER ISOLATED</span><span><i className="off"/> LIVE LOCKED</span></div><div className="top-actions"><button onClick={() => setPalette(true)}><Search/><span>Search</span><kbd>Ctrl K</kbd></button><button className="icon-btn"><Bell/></button><button className="icon-btn desktop" onClick={() => setKiosk(value => !value)} aria-label="Toggle kiosk mode"><PanelLeftClose/></button><button className="icon-btn" onClick={signOut} aria-label="Sign out"><LogOut/></button><span className="avatar">{(account?.user.display_name || account?.user.email || 'MM').slice(0, 2).toUpperCase()}</span></div></header>
    <main>{children}</main><nav className="mobile-nav">{visibleNav.slice(0, 5).map(([href, label, Icon]) => <Link key={href} href={href} className={path === href ? 'active' : ''} onClick={() => markNavigationStart(href)}><Icon/><small>{label.split(' ')[0]}</small></Link>)}</nav>
